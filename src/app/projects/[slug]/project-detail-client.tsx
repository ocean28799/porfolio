      ...
      <FullScreen className="w-full max-w-7xl mx-auto py-6 pt-16 sm:pt-6 xs:py-4 px-4 xs:px-2 relative z-10">
        ...
        <div className="grid xs:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 xs:gap-6 pt-6 xs:pt-4">
          ...
          <div className="xs:col-span-1 lg:col-span-1 xl:col-span-2 space-y-8 xs:space-y-6">
            ...
            {project.challenge && project.solution && (
              <motion.div
                ...
                className="grid md:grid-cols-2 gap-4 xs:gap-3"
              >
                {/* The Challenge */}
                <div className="bg-gradient-to-br from-red-900/20 to-orange-900/20 backdrop-blur-sm border border-red-500/20 rounded-xl xs:rounded-lg p-5 xs:p-4">
                  <div className="flex items-center gap-2 xs:gap-1.5 mb-3 xs:mb-2">
                    <div className="w-5 h-5 xs:w-4 xs:h-4 bg-gradient-to-r from-red-500 to-orange-500 rounded-lg xs:rounded flex items-center justify-center">
                      <Lightbulb className="w-2.5 h-2.5 xs:w-2 xs:h-2 text-white" />
                    </div>
                    <h3 className="text-lg xs:text-base font-bold text-white">The Challenge</h3>
                  </div>
                  <p className="text-slate-300 leading-relaxed text-sm xs:text-xs">{project.challenge}</p>
                </div>

                {/* The Solution */}
                <div className="bg-gradient-to-br from-green-900/20 to-emerald-900/20 backdrop-blur-sm border border-green-500/20 rounded-xl xs:rounded-lg p-5 xs:p-4">
                  <div className="flex items-center gap-2 xs:gap-1.5 mb-3 xs:mb-2">
                    <div className="w-5 h-5 xs:w-4 xs:h-4 bg-gradient-to-r from-green-500 to-emerald-500 rounded-lg xs:rounded flex items-center justify-center">
                      <CheckCircle className="w-2.5 h-2.5 xs:w-2 xs:h-2 text-white" />
                    </div>
                    <h3 className="text-lg xs:text-base font-bold text-white">The Solution</h3>
                  </div>
                  <p className="text-slate-300 leading-relaxed text-sm xs:text-xs">{project.solution}</p>
                </div>
              </motion.div>
            )}
              ...
              >
                <div className="flex items-center gap-2 xs:gap-1.5">
                  <div className="w-6 h-6 xs:w-5 xs:h-5 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-lg xs:rounded flex items-center justify-center">
                    <BarChart3 className="w-3 h-3 xs:w-2.5 xs:h-2.5 text-white" />
                  </div>
                  <h2 className="text-2xl xs:text-xl sm:text-3xl font-bold text-white">Key Results</h2>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 xs:gap-2">
                  {project.results.map((result, index) => (
                    <motion.div
                      ...
                      className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl xs:rounded-lg p-4 xs:p-3 text-center hover:bg-slate-800/70 transition-all"
                    >
                      <div className="text-2xl xs:text-xl sm:text-3xl font-bold bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">
                        {result.value}
                      </div>
                      <div className="text-xs xs:text-[10px] text-slate-400 mt-1">{result.metric}</div>
                      {result.improvement && (
                        <div className="text-xs xs:text-[10px] text-green-400 mt-1 font-medium">
                          {result.improvement}
                        </div>
                      )}
                    </motion.div>
            ...
            {project.architecture && project.architecture.length > 0 && (
              ...
              >
                <div className="flex items-center gap-2 xs:gap-1.5">
                  <div className="w-6 h-6 xs:w-5 xs:h-5 bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg xs:rounded flex items-center justify-center">
                    <Layers className="w-3 h-3 xs:w-2.5 xs:h-2.5 text-white" />
                  </div>
                  <h2 className="text-2xl xs:text-xl sm:text-3xl font-bold text-white">Architecture</h2>
                </div>
                <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl xs:rounded-lg p-5 xs:p-4">
                  <div className="flex flex-wrap gap-3 xs:gap-2 justify-center">
                    {project.architecture.map((item, index) => (
                      ...
                      >
                        <div className="px-3 xs:px-2 py-2 xs:py-1.5 bg-gradient-to-r from-purple-900/50 to-pink-900/50 border border-purple-500/30 rounded-lg xs:rounded text-sm xs:text-xs font-medium text-purple-200">
                          {item}
                        </div>
          ...
          <div className="xs:col-span-1 space-y-6 xs:space-y-4">
            {/* Project Info Card */}
            {(project.timeline || project.role || project.teamSize) && (
              <motion.div
              ...
              >
                <div className="flex items-center gap-2 xs:gap-1.5 mb-4 xs:mb-3">
                  <div className="w-5 h-5 xs:w-4 xs:h-4 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-lg xs:rounded flex items-center justify-center">
                    <Calendar className="w-2.5 h-2.5 xs:w-2 xs:h-2 text-white" />
                  </div>
                  <h3 className="text-lg xs:text-base font-bold text-white">Project Info</h3>
                </div>
                <div className="space-y-3 xs:space-y-2">
                  {project.timeline && (
                    <div className="flex items-center gap-2 xs:gap-1.5">
                      <Clock className="w-4 h-4 xs:w-3.5 xs:h-3.5 text-blue-400" />
                      <div>
                        <div className="text-xs xs:text-[11px] text-slate-400">Timeline</div>
                        <div className="text-sm xs:text-xs font-semibold text-slate-200">{project.timeline}</div>
                      </div>
                    </div>
                  )}
                  {project.role && (
                    <div className="flex items-center gap-2 xs:gap-1.5">
                      <UserCircle className="w-4 h-4 xs:w-3.5 xs:h-3.5 text-purple-400" />
                      <div>
                        <div className="text-xs xs:text-[11px] text-slate-400">My Role</div>
                        <div className="text-sm xs:text-xs font-semibold text-slate-200">{project.role}</div>
                      </div>
                    </div>
                  )}
                  {project.teamSize && (
                    <div className="flex items-center gap-2 xs:gap-1.5">
                      <Users className="w-4 h-4 xs:w-3.5 xs:h-3.5 text-green-400" />
                      <div>
                        <div className="text-xs xs:text-[11px] text-slate-400">Team Size</div>
                        <div className="text-sm xs:text-xs font-semibold text-slate-200">{project.teamSize}</div>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {project.highlights && project.highlights.length > 0 && (
              <motion.div
                ...
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: 0.55 }}
                className="bg-gradient-to-br from-yellow-900/20 to-orange-900/20 backdrop-blur-sm border border-yellow-500/20 rounded-xl xs:rounded-lg p-4 xs:p-3"
              >
                <div className="flex items-center gap-2 xs:gap-1.5 mb-4 xs:mb-3">
                  <div className="w-5 h-5 xs:w-4 xs:h-4 bg-gradient-to-r from-yellow-500 to-orange-500 rounded-lg xs:rounded flex items-center justify-center">
                    <Trophy className="w-2.5 h-2.5 xs:w-2 xs:h-2 text-white" />
                  </div>
                  <h3 className="text-lg xs:text-base font-bold text-white">Highlights</h3>
                </div>
                <div className="space-y-2 xs:space-y-1.5">
                  {project.highlights.map((highlight, index) => (
                    <div key={index} className="flex items-start gap-2 xs:gap-1.5">
                      <Star className="w-3 h-3 xs:w-2.5 xs:h-2.5 text-yellow-400 mt-1 flex-shrink-0" />
                      <span className="text-sm xs:text-xs text-slate-300">{highlight}</span>
                    </div>
                  ))}
                </div>
              </motion.div>
            ...
            >
              <div className="flex items-center gap-2 xs:gap-1.5 mb-4 xs:mb-3">
                ...
                <h3 className="text-lg xs:text-base font-bold text-white">Quick Stats</h3>
              </div>
              <div className="space-y-4 xs:space-y-3">
                ...
                <div className="flex items-center gap-2 xs:gap-1.5">
                  ...
                  <div>
                    ...
                    <div className="text-lg xs:text-base font-semibold text-slate-200">{project.timeline || "3-6 months"}</div>
                  </div>
                </div>
              </div>
            </motion.div>
